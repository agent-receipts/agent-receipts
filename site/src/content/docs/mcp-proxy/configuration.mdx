...
### `-32002: approval timeout or rejected` on specific tools

**Symptom:** a particular subset of tools fails with JSON-RPC error `-32002`, while the rest of the same server works. Common triggers are tools whose names contain `auth`/`token`/`secret`/`key`/`credential`/`password`, tools named `*config*` or `*setting*` combined with writes, or SQL execution tools called with mutation strings that omit `WHERE`. Ordinary GitHub writes like `create_pull_request` and `push_files` are unaffected.

**Cause:** the default policy contains `pause_high_risk` (`min_risk_score: 50`). The scorer adds +30 for sensitive keywords, +30 for SQL mutations without `WHERE`, +20 for `config`/`setting`, and a 20–40 base by operation type — so those tools cross the 50 threshold and get paused. If nothing is POSTing `/api/tool-calls/{id}/approve`, every paused call sits until `-approval-timeout` elapses (default 60 s) and is then auto-denied with `-32002`. Everything below the threshold flows through normally, which is why the failure looks tool-specific.
