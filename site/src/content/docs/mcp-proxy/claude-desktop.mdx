...
:::caution[Approval server needed if the policy pauses]
With the default policy, calls scoring ≥50 (auth/token/secret/key/credential tools, SQL mutations without `WHERE`) are paused until approved. The config below pins `-http 127.0.0.1:8080` so an approver has a stable URL. Skip the approver and any paused call fails with `-32002: approval timeout or rejected` after the approval timeout, while lower-scoring writes (`create_pull_request`, `push_files`) still succeed — making the failure look tool-specific. See [Approval Server](/mcp-proxy/approval-ui/).
:::
...
`-http 127.0.0.1:8080` pins the approval endpoint to a known port so an approver can target it. If the policy ever pauses a call and no approver responds, it fails with `-32002: approval timeout or rejected` — see [Approval Server](/mcp-proxy/approval-ui/). If you also run Claude Code or Codex simultaneously, give each client a distinct port (e.g. `8080`, `8081`, `8082`).
