...
:::caution[Approval server is required when the policy pauses calls]
The built-in policy contains a `pause` rule (`pause_high_risk`, `min_risk_score: 50`). Tool calls that score at or above 50 — things like `update_auth_config`, `create_api_key`, `delete_credential`, and SQL mutations without a `WHERE` clause — are held until an approver POSTs a decision to the proxy's HTTP endpoint. Without an approver, any such call fails with `-32002: approval timeout or rejected` after the approval timeout, while lower-scoring calls (ordinary reads and writes like `create_pull_request`, `push_files`, `create_branch`) still pass — so a failure can look tool-specific.

Most GitHub MCP tools don't reach 50 under the default scorer, so many deployments never trigger a pause in practice. But any time the policy *can* pause a call, you should plan for it. Either:
