...
The built-in defaults include a `pause_high_risk` rule (`min_risk_score: 50`). A call only pauses when its score actually reaches 50, which under the current scorer means tools with names containing `auth`/`token`/`secret`/`key`/`credential`/`password`, `*config*`/`*setting*` tools combined with writes, SQL mutations without `WHERE`, or similar. Plenty of everyday calls (reads, ordinary writes like `create_pull_request`, `push_files`) score below the threshold and don't need approval.
