...
before_tool_call (priority 100)
  └─ stash { toolName, params, paramsHash, startedAt }

[tool executes]

after_tool_call
  └─ classify action type and risk level
  └─ build receipt (action, outcome, principal, chain position)
  └─ sign with Ed25519
...
| `write_file`, `write` | `filesystem.file.modify` | medium |
...
Every receipt is a W3C Verifiable Credential. Here's a full receipt from the v0 trial — the first `system.command.execute` in the session:
...
Rows 8 and 10 are the agent — unprompted — calling `ar_query_receipts` mid-session to inspect its own audit trail. Both calls have receipts. The receipt for the audit query is in the audit trail.
...
- **Subagent chain linking** — when an agent fans out into parallel subagents, linking their receipt chains to the parent. ([ar#14](https://github.com/agent-receipts/ar/issues/14))
...
See [OpenClaw Installation](/openclaw/installation/) to get started.
